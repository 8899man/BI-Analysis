...
//				$scope.citySelected = $scope.areas[0]["subs"][0];
		...
		$scope.changeArea = function(){
			ownerTypeServices.getCompanys({areaCode: $scope.areaSelected.areaCode} , function(response) {
...
//			$scope.error.city = $scope.citySelectedSearch ? false : true;
			...
			if(!$scope.error.area && $scope.areaSelected.areaName == "集团"){
				$scope.error.city = false;
			}
...
//			}
			...
			var _info = {
				areaCode : $scope.areaSelectedSearch.areaCode,
				areaName : $scope.areaSelectedSearch.areaName,
				companyCode : $scope.citySelectedSearch.companyCode,
