define(['app', 'jquery', 'salesQuery', 'toastr', "salesQueryService"], function (app) {
    //console.dir(app);
	app.register.controller('SalesQueryController', ['$location', '$modal', '$filter', '$scope', '$rootScope', '$state', 'salesQuery', 'toastr', "salesQueryService", "$stateParams", function ($location, $modal, $filter, $scope, $rootScope, $state, salesQuery, toastr, salesQueryService, $stateParams) {
		...
		$scope.initializeController = function () {
			...
			//获取数据
			salesQueryService.getHouses({"projGUID": $stateParams.projectId},
					$scope.getSalesQuerySuccess, $scope.getSalesQueryError);
		};
		...
		$scope.search = function(){
			//console.log("search");
			$scope.houses = $scope.unit.houses;
			$scope.houses = $filter('filter')($scope.houses, {'room': $scope.houseNumber})
		}
