...
public class LoginAction extends BaseAction {
	...
	public @ResponseBody
	ResultMessage login(String username, String password, String code) {

		/*
		 * // 判断验证是否通过 if (errors.hasErrors()) { StringBuilder sb = new
		 * StringBuilder(); for (FieldError e : errors.getFieldErrors()) { if
		 * (sb.length() > 0) { sb.append(","); }
		 * sb.append(e.getDefaultMessage()); break; } return
		 * ResultMessage.errorMsg(sb.toString()); }
		 * 
		 * String sessionRand = this.getStr("random");
		 * 
		 * sessionRand = "1234";
		 * 
		 * // 验证码只能使用一次 this.set("random", null);
		 * 
		 * if (StringUtils.isBlank(sessionRand)) { return
		 * ResultMessage.errorMsg("验证码已过期"); }
		 * 
...
//			return ResultMessage.errorMsg("验证码不为空!");
//		}
//		
...
//		}
		...
		// 管理员
		if (user.getUserType() == 99 || user.getUserType() == 1) {

			// 获取权限
			Set right_set = loginService.getSysRightSet();
			Set data_set = loginService.getDataRightSet();
			// 设置权限
			this.set("rightSet", right_set);
			this.set("dataRightSet", data_set);
			...
			Set data_set = loginService.getDataRightSetByUser(user.getUserId());
			// 设置权限
			this.set("rightSet", right_set);
			this.set("dataRightSet", data_set);
			
			List<EMenu> menuList = loginService.getEMenuUser(user.getUserId());
			map.put("menu", menuList);
			map.put("user", baseUser);
		}
	...
	ResultMessage initData() {
		...
		try {
			...
			if (baseUser.getUserType() == 99 || baseUser.getUserType() == 1) {
				
				List<EMenu> menuList = loginService.getEMenuAll();

				map.put("menu", menuList);
				map.put("user", baseUser);
				map.put("isAuthenicated", "true");
				return ResultMessage.successMsg("成功", map);

			} else {

				List<EMenu> menuList = loginService.getEMenuUser(baseUser
						.getUserId());
				map.put("menu", menuList);
				map.put("user", baseUser);
				map.put("isAuthenicated", "true");
