...
public class ReportsService {
	...
	public Map<String,Map> ownerTypeData(Map params) {
		...
		List<Map> cityList = getCityList( reportsDao.getWorkareaListByAreaCode(params) );
	...
	private  List<Map> getCityList(List cityList) {
		...
		for(int i=0; i<cityList.size(); i++) {
			Map work_area = (Map)cityList.get(i);
			Map tmp = new HashMap();
			tmp.put("name", (String)work_area.get("work_area"));
			if ("未知".equals(work_area)) {
				tmp.put("alisName", "城市未知");
			} else {
				tmp.put("alisName", (String)work_area.get("work_area"));
			}
			retValue.add(tmp);
		}
		return retValue ;
	...
	public Map turnoverCustomerData(Map params) {
		...
		List<Map> productList = getProduceList(BusiConfig.getProductList());
		...
		retValue.put("productList", productList);
	...
	private List<Map> getProduceList(List<String> productList) {
		List<Map> retValue = new ArrayList<Map>();
		for(int i=0; i<productList.size(); i++) {
			String product = (String)productList.get(i);
			Map tmp = new HashMap();
			tmp.put("name", product);
			if ("未知".equals(product)) {
				tmp.put("alisName", "产品类型未知");
			} else {
				tmp.put("alisName", product);
			}
			retValue.add(tmp);
		}
		return retValue;
	...
	private List<Map> getGroupItems(Map params) {
		...
		if ("group".equals(unitType)) {
			params.put("fieldName", "big_code");
			params.put("unitType", "area");
//----------------------			List<BAreaCompany> areas = bAreaCompanyDao.getAreaList();
			paramMap.put("subType", 2);
			paramMap.put("parentGUID", null);
			List<BAreaCompany> areas = bOrgDao.getAreaOrCompany(paramMap);
			for (int i=0; i<areas.size(); i++) {
				BAreaCompany area = areas.get(i);
				Map tmp = new HashMap();
				tmp.put("name", area.getName());
				tmp.put("alisName", area.getName());
				tmp.put("code", area.getId());
				retValue.add(tmp);
			}
		} else if ("area".equals(unitType)) {
			params.put("fieldName", "city_code");
			params.put("unitType", "city");
			
//---------------------------------			List<BAreaCompany> companies = bAreaCompanyDao.getCompanyList((String)params.get("countCode"));
			paramMap.put("subType", 3);
			paramMap.put("parentGUID", (String)params.get("countCode"));
			List<BAreaCompany> companies = bOrgDao.getAreaOrCompany(paramMap);
			for (int i=0; i<companies.size(); i++) {
				BAreaCompany company = companies.get(i);
				Map tmp = new HashMap();
				tmp.put("name", company.getName());
				tmp.put("alisName", company.getName());
				tmp.put("code", company.getId());
				retValue.add(tmp);
			}
...
//			prject_name, projGUID
	...
	public Map getRepeatBuyTiems(Map params) {
		Map retValue = new HashMap();
		...
		System.out.println(params);
		List<Map> list = reportsDao.getRepeatBuyTiems(params);
		
		retValue.put("repeatBuyTiems", list);
		retValue.put("groupItems", groupItems);
		retValue.put("huxingList", huxingList);
		retValue.put("mianjiList", mianjiList);
		retValue.put("ageList", ageList);
