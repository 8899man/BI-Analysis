define(['app', 'window', 'jquery', 'directedServices', 'toastr'], function (app, w) {
	...
	$scope.initializeController = function () {
		...
		$scope.banner.active = 1;//默认显示banner第三张
...
//			$scope.analysisAreas = response.areas;
	...
	$scope.changeAnalysisArea = function(area){
		...
		$scope.getProcitys({
			provinceId:$scope.analysisAreaCurrent.value
		});
	...
	//通过省份id获取省下的城市
	$scope.getProcitys = function(info){
		directedServices.getProcitys(info, function(response, status){
			...
			if(response.header.code != 0){
				...
				$scope.analysisCities = response.data.proCitys;
				$scope.analysisCityCurrent = $scope.analysisCities[0];
			}
		}, function(response, status){
			toastr.warning("操作失败", '提示', {
