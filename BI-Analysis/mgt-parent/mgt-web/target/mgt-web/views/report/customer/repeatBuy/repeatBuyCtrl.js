...
//			$scope.searchData();
		...
		$scope.searchBack = function(item){
			...
			$scope.setData($scope.lastRepeatBuyTiemsData);
...
//				$scope.unitSearch = $scope.unitCity;
//			}
			//location.reload();
		}
...
//			$scope.unit = unit;
			...
			$scope.lastRepeatBuyTiemsData = {
					repeatBuyTiems: $scope.repeatBuyTiems || {}, // 将当前值赋给上次结果变量中
					groupItems: $scope.groupItems || {},
					huxingList: $scope.huxingList || {},
					mianjiList: $scope.mianjiList || {},
					ageList: $scope.ageList || {},
					sexList: $scope.sexList || {},
					productList: $scope.productList || {}
			};
			$scope.lastUnitSearch = $scope.unitSearch || {};
...
//			$scope.unitSearch = $scope.unit;
			...
			repeatBuyServices.searchData(info, function(response, status){
				...
				$scope.repeatBuyTiems = response.data.repeatBuyTiems;
...
//				}
		...
		$scope.setData = function(params){
			...
			//设置数据列表
			$scope.list = params.repeatBuyTiems;
			$scope.repeatBuyTiems = params.repeatBuyTiems;
...
//			$scope.chartConfigObj2 = null;
			...
			//if (chart3.categories.length > 0 && chart3.series.length > 0) {
